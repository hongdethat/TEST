...
import './css/Post.css';
import { camxuc, Bon, troichay, chungkhoan } from '../../images/images';
...
const BlogsDATA = Data.slice(0, loadBlog).map((blog, i) => {
    return (
        <div className={blog.css} key={i}>
            ...
            <div className="Section_post_row">
              <p className="Post_flex"><img src={blog.img} alt="Data"/>{t(blog.data)}</p>
              <p>{blog.active}</p>
            </div>
  ...
  <div className="Section_post_padding" id="Section_post">
    <span className="PostName">
      ...
      <div disabled={Data.length === BlogsDATA.length}
                type="button"
                onClick={() => setloadBlog((LOAD) => LOAD + 4)}
                className="Section_post_button">
          Xem thêm <HiOutlineArrowSmRight/>
      </div>
    </span>
